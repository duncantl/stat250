...
public class DelaysFrequencyCombinerTable {
	...
	job.setJarByClass(DelaysFrequencyCombinerTable.class);
	job.setJobName("Delay Frequency Table with Combiner");

	FileInputFormat.addInputPath(job, new Path(args[0]));
	FileOutputFormat.setOutputPath(job, new Path(args[1]));

	job.setMapperClass(ArrivalDelayMapper.class);
	job.setCombinerClass(ArrivalDelayCombiner.class);
	job.setReducerClass(ArrivalDelayReduceCombine.class);
