...
# http://sites.stat.psu.edu/~mharan/MCMCtut/MCMC.html
    ...
    for (i in 2:NUMIT)
      {
        ...
        logMHratio = sum(Y[1:propk])*log(currtheta)+
                      sum(Y[(propk+1):n])*log(currlambda) -
                       propk*currtheta- (n-propk)*currlambda -
                        (sum(Y[1:currk])*log(currtheta)+sum(Y[(currk+1):n])*log(currlambda) -
...
#        currk = KGUESS # if we do not sample k (k fixed)
    ...
    rownames(mchain) = c("theta", "lambda", "k", "b1", "b2")
    
    return(as.data.frame(t(mchain)))
  }
